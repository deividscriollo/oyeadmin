angular.module('scotchApp').controller('carteraController', function ($scope, $route) {
	...
	$scope.methodpagos = function(id) { 
		$.ajax({
			url: 'data/cartera/app.php',
			type: 'post',
			data: {llenar_tabla_pagos:'llenar_tabla_pagos',id:id},
			success: function (data) {
				$('#tbt_pagos tbody').html(data);
			}
		});
		...
		$.ajax({
			...
			success: function(response) { 
				...
				for (var i = 0; i < response.length; i++) {
					var vizualizar = "<button type='button' class='btn btn-white btn-primary btn-bold btn-sm' onclick=\"angular.element(this).scope().methodpagos('"+response[i].id+"')\" data-toggle='tooltip' title = 'Visualizar Pagos'><span class='fa fa-eye'> VISUALIZAR</button>";
					var estado = "<span class='label label-success arrowed-in arrowed-in-right'>ABIERTO</span>";
