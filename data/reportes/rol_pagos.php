...
function Header() {
    ...
    $resp = $class->consulta("SELECT P.id, P.nombres, P.apellidos, P.cedula_identificacion, R.fecha, R.sueldo, R.neto_pagar, R.codigo, D.tiempos_horas, D.dias_laborados, D.horas_trabajadas, D.sueldo_basico, D.horas_extras, D.comisiones, D.decimo_tercero, D.decimo_cuarto, D.total_ingresos, D.aporte_iess, D.p_quirografarios, D.p_anticipos, D.atrasos, D.permisos, D.total_descuentos, D.faltas, R.fecha_creacion, D.dias_no_laborados FROM corporativo.personal P, rol_pagos.rol_pagos  R, rol_pagos.detalle_rol_pagos D where R.id = D.id_rol_pagos and P.id = R.id_personal and R.id = '".$_GET['id']."'");
    while ($row = $class->fetch_array($resp)) {
        ...
        $tiempos_horas = $row[8];
        $dias_laborados = $row[9];
        $horas_trabajadas = $row[10];
        $sueldo_basico = $row[11];
        $horas_extras = $row[12];
        $comisiones = $row[13];
        $decimo_tercero = $row[14];
        $decimo_cuarto = $row[15];
        $total_ingresos = $row[16];
        $aporte_iess = $row[17];
        $prestamos_qui = $row[18];
        $prestamos_antici = $row[19];
        $atrasos = $row[20];
        $permisos = $row[21];
        $total_descuentos = $row[22];
        $faltas = $row[23];
        $fecha_creacion = substr($row[24], 10, 9);
        $dias_no_laborados = $row[25];
    }

    $resp = $class->consulta("SELECT C.nombre FROM corporativo.cargos_personal G, corporativo.cargo C, corporativo.personal P where P.id = G.id_personal and C.id = G.id_cargo  and G.id_personal = '".$id_personal."'");
    while ($row = $class->fetch_array($resp)) {
        $cargo = $row[0];
    }
    ...
    $this->Text(39, 50, maxCaracter(utf8_decode($nombres).' '.utf8_decode($apellidos),23),1,0, 'L',0);/////nombres completos
    $this->SetFont('Arial','B',9);
